...
import { Node, NATStaticOutboundRule, Vendor, NATPortMappingRule, NATAddressPool, NATMappingType, NATServerGroup, NATServerGroupMember, HuaweiNATConfig, HuaweiNATAddressPoolSection, HuaweiNATServer, DeviceType, H3CGlobalNatRule, SecurityZone, AddressGroup, ServiceGroup } from '../../types';
...
}> = ({ rule, onUpdate, zones, addressGroups, serviceGroups }) => {
    return (
...
const Huawei_NATConfig: React.FC<VendorNATConfigProps> = ({ selectedNode, onNodeUpdate, isGenerating }) => {
    ...
    const addAddressPool = () => {
        const newPool = { id: `hpool-${Date.now()}`, groupName: `pool${huaweiConfig.addressPools.length + 1}`, sections: [{id: `hsec-${Date.now()}`, startAddress: ''}], mode: 'pat' as const, routeEnable: false };
        updateHuaweiConfig({ addressPools: [...huaweiConfig.addressPools, newPool] });
    };

    const addRule = () => {
        const newRule = { id: `hrule-${Date.now()}`, ruleName: `rule${huaweiConfig.rules.length + 1}`, action: 'source-nat' as const };
    ...
    return (
        <div className="space-y-4">
            <div className="flex border-b border-slate-600 text-xs">
                <button onClick={() => setActiveTab('source')} className={`px-3 py-2 ${activeTab === 'source' ? 'border-b-2 border-blue-500 text-white' : 'text-slate-400'}`}>源NAT</button>
                <button onClick={() => setActiveTab('destination')} className={`px-3 py-2 ${activeTab === 'destination' ? 'border-b-2 border-blue-500 text-white' : 'text-slate-400'}`}>目的NAT</button>
            </div>
            {activeTab === 'source' && (
                <div className="space-y-4">
                    <div className="space-y-2">
                        ...
                        {huaweiConfig.addressPools.map((pool, index) => (
                            <div key={pool.id} className="bg-slate-800/50 p-2 rounded text-xs">
                                <div className="flex justify-between items-center cursor-pointer" onClick={() => setEditingItemId(editingItemId === pool.id ? null : pool.id)}><span>{pool.groupName}</span><button onClick={(e)=>{e.stopPropagation(); removePool(index)}} className="px-2 py-1 bg-red-600 text-white text-xs rounded">X</button></div>
                            </div>
                        ))}
                    </div>
                     <div className="space-y-2">
                        ...
                        {huaweiConfig.rules.map((rule, index) => (
                            <div key={rule.id} className="bg-slate-800/50 p-2 rounded text-xs">
                                <div className="flex justify-between items-center cursor-pointer" onClick={() => setEditingItemId(editingItemId === rule.id ? null : rule.id)}><span>{rule.ruleName}</span><button onClick={(e)=>{e.stopPropagation(); removeRule(index)}} className="px-2 py-1 bg-red-600 text-white text-xs rounded">X</button></div>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="flex justify-between items-center"><h5 className="text-sm font-medium text-slate-300">NAT Server (服务器映射)</h5><button onClick={addServer} className="px-2 py-1 bg-green-600 text-white text-xs rounded">添加服务器</button></div>
                     {huaweiConfig.servers.map((server, index) => (
                        <div key={server.id} className="bg-slate-800/50 p-2 rounded text-xs">
                             <div className="flex justify-between items-center cursor-pointer" onClick={() => setEditingItemId(editingItemId === server.id ? null : server.id)}><span>{server.name}</span><button onClick={(e)=>{e.stopPropagation(); removeServer(index)}} className="px-2 py-1 bg-red-600 text-white text-xs rounded">X</button></div>
                        </div>
                    ))}
                </div>
            )}
