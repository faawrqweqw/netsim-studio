...
import { RouterIcon, L2SwitchIcon, L3SwitchIcon, PCIcon, FirewallIcon, APIcon, ACIcon, ServerIcon, TextBoxSolidIcon, RectangleIcon, CircleIcon, RotateIcon } from './Icons';
        ...
        >
            <svg className="absolute top-0 left-0 w-full h-full pointer-events-none z-0">
                {Object.values(connectionGroups).map((group: Connection[]) => {
                    // Fix: Explicitly type `group` as Connection[] to resolve type inference issues.
                    ...
                    // Fix: Check if group is an array before mapping to avoid errors
                    return (Array.isArray(group) ? group : []).map(conn => {
                        const fromPos = getSmartEdgePoint(fromNode, toNode);
                        const toPos = getSmartEdgePoint(toNode, fromNode);
                        const dx = toPos.x - fromPos.x;
                        const dy = toPos.y - fromPos.y;
