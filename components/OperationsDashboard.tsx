...
const OperationsDashboard: React.FC<OperationsDashboardProps> = ({ nodes, onNodeUpdate, managedDevices, onManagedDevicesUpdate }) => {
    ...
    const operationalDevices: OperationalDevice[] = useMemo(() => {
        // FIX: Correctly map Node to OperationalDevice by explicitly selecting properties to match the target type.
        const topologyDevices: OperationalDevice[] = nodes
            .filter(n => n.config.management.ipAddress && n.vendor !== Vendor.Generic)
            .map((n): OperationalDevice => ({ // Added explicit return type for clarity
                id: n.id,
                name: n.name,
                vendor: n.vendor,
                type: n.type,
        ...
        // FIX: Explicitly map ManagedDevice to OperationalDevice for type safety and consistency.
        const inventoryDevices: OperationalDevice[] = managedDevices.map((d): OperationalDevice => ({ // Added explicit return type for clarity
            id: d.id,
            name: d.name,
            vendor: d.vendor,
            type: d.type,
