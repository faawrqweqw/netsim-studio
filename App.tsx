...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const onWheel = (e: WheelEvent) => {
            ...
            setAppState(prev => {
                const activeTopo = prev.topologies[prev.activeTopologyId];
                if (!activeTopo) return prev;
    
                const { canvasScale, canvasTranslate } = activeTopo;
                ...
                const newActiveTopo = {
                    ...
                    canvasTranslate: { x: newTranslateX, y: newTranslateY }
                };
                ...
                return {
                    ...
                    topologies: {
                        ...prev.topologies,
                        [prev.activeTopologyId]: newActiveTopo
                    }
                };
            });
        };
    ...
    const handleNewTopology = useCallback(() => {
        setAppState(prev => {
            ...
            const topoNumbers = Object.values(prev.topologies)
                // FIX: Add explicit type `Topology` to map callback parameter to resolve 'property does not exist on type unknown' error.
                .map((t: Topology) => t.name.match(/^Topology (\d+)$/))
