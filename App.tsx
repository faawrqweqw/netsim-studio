...
import { Node, Connection, DeviceType, Vendor, Port, LinkConfig, ManagedDevice } from './types';
                            ...
                            {section.items.map((item) => {
                                ...
                                if (item.tool === 'connect') {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="w-8 h-8 text-purple-400 flex items-center justify-center">{item.icon}</div>
...
const App: React.FC = () => {
    ...
    const [managedDevices, setManagedDevices] = useState<ManagedDevice[]>(() => {
        try {
            const savedData = localStorage.getItem('netsim-studio-data');
            return savedData ? JSON.parse(savedData).managedDevices || [] : [];
        } catch (error) {
            console.error("Failed to load managed devices from localStorage:", error);
            return [];
        }
    });
        ...
        const debounceSave = setTimeout(() => {
            try {
                const dataToSave = JSON.stringify({ 
                    nodes: nodes.map(({ runtime, ...rest }) => rest), 
                    connections, 
                    managedDevices: managedDevices.map(({ runtime, ...rest }) => rest) 
                });
    ...
    }, [nodes, connections, managedDevices]);
    ...
    return (
        <div className="flex h-screen w-screen font-sans bg-slate-900">
            <MainSidebar currentView={view} onViewChange={setView} />
                 ...
                 <>
                    ...
                    <main className="flex-1 flex flex-col bg-slate-900 relative">
                        ...
                        <div className="absolute top-2 right-2 flex items-center gap-2 z-20 bg-slate-800/50 border border-slate-700 p-1 rounded-lg">
                            ...
                            <button onClick={clearCanvas} className="bg-slate-700 hover:bg-slate-600 p-2 rounded-md transition-colors" title="清除画布"><BroomIcon className="w-5 h-5"/></button>
            ...
            {view === 'operations' && (
                <OperationsDashboard 
                    ...
                    onManagedDevicesUpdate={setManagedDevices}
                />
            )}
        </div>
    );
