...
import { Node, ManagedDevice, OperationalDevice, Vendor, DeviceType } from '../types';
...
const OperationsDashboard: React.FC<OperationsDashboardProps> = ({ nodes, onNodeUpdate, managedDevices, onManagedDevicesUpdate }) => {
    ...
    const operationalDevices: OperationalDevice[] = useMemo(() => {
        ...
        const inventoryDevices: OperationalDevice[] = managedDevices.map(d => ({
            id: d.id,
            name: d.name,
            vendor: d.vendor,
            type: d.type,
    ...
    const handleUpdateDevice = useCallback((updatedDevice: OperationalDevice) => {
        if (updatedDevice.source === 'topology') {
            ...
            const updatedManagedDevices = managedDevices.map(d => 
                d.id === updatedDevice.id 
                ? { ...d, name: updatedDevice.name, management: updatedDevice.management, runtime: updatedDevice.runtime }
                : d
            );
