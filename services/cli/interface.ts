import { InterfaceIPConfig, PhysicalInterfaceIPConfig, ACL, Vendor } from '../../types';

export const generateInterfaceIpCli = (vendor: string, deviceType: string, config: InterfaceIPConfig, acls: ACL[] = []): { cli: string; explanation: string } => {
        ...
        if (intf.enableDHCP) {
             if (vendorLower === 'cisco') {
                if (intf.dhcpMode === 'relay' && intf.dhcpServerIP) {
                    interfaceBlock += ` ip helper-address ${intf.dhcpServerIP}\n`;
                }
             } else if (vendorLower === 'huawei') {
                 if (intf.dhcpMode === 'relay' && intf.dhcpServerIP) {
                    interfaceBlock += ` dhcp select relay\n`;
                    interfaceBlock += ` dhcp relay server-ip ${intf.dhcpServerIP}\n`;
                } else if (intf.dhcpMode === 'global' && intf.selectedPool) {
                    interfaceBlock += ` dhcp select global pool ${intf.selectedPool}\n`;
                ...
                if (intf.dhcpMode === 'relay' && intf.dhcpServerIP) {
                    interfaceBlock += ` dhcp select relay\n`;
                    interfaceBlock += ` dhcp relay server-select ${intf.dhcpServerIP}\n`;
                } else if (intf.dhcpMode === 'global' && intf.selectedPool) {
