...
const generateH3cGlobalPolicyCli = (nodeConfig: Node['config']): string => {
    ...
    globalPolicy.rules.forEach(rule => {
        ...
        // Matching conditions
        if (rule.sourceZone) cli += `  source-zone ${rule.sourceZone}\n`;
        if (rule.destinationZone) cli += `  destination-zone ${rule.destinationZone}\n`;
...
export const generateNatCli = (vendor: string, deviceType: string, nodeConfig: Node['config']): { cli: string; explanation: string } => {
    ...
    if (config.staticOutbound.enabled && config.staticOutbound.rules.length > 0) {
        if (vendorLower === 'h3c') {
            ...
            const staticCli = `# Static NAT for ${vendor} is not yet supported.`;
            globalCliParts.push(staticCli);
        }
    }

    if (vendorLower === 'h3c') {
        const globalPolicyCli = generateH3cGlobalPolicyCli(nodeConfig);
        if (globalPolicyCli) {
