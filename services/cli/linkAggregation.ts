...
export const generateLinkAggregationCli = (vendor: Vendor, deviceType: DeviceType, config: LinkAggregationConfig): { cli: string; explanation: string } => {
    const { 
        groupId, mode, members, description, loadBalanceAlgorithm, 
        interfaceMode, accessVlan, trunkNativeVlan, trunkAllowedVlans, 
        systemPriority, preemptEnabled, preemptDelay, timeout 
    } = config;
    ...
    if (vendor === Vendor.Cisco) {
        if (loadBalanceAlgorithm) {
            globalCli += `port-channel load-balance ${loadBalanceAlgorithm}\n`;
        }
    ...
    // --- Huawei ---
        ...
        if (mode === 'lacp-static') {
            if (config.huaweiLacpPriorityMode === 'system-priority') {
                globalCli += 'lacp priority-command-mode system-priority\n';
            }
            if (systemPriority && systemPriority !== '32768') {
                if (config.huaweiLacpPriorityMode === 'system-priority') {
                    globalCli += `lacp system-priority ${systemPriority}\n`;
                } else { // default mode
                    globalCli += `lacp priority ${systemPriority}\n`;
                }
            }
        }
        ...
        aggInterfaceCli += `interface ${aggIntfName}\n`;
        if (mode === 'manual') {
            aggInterfaceCli += ` mode manual load-balance\n`;
        } else if (mode === 'lacp-static') {
            aggInterfaceCli += ` mode lacp-static\n`;
        }
        ...
        if (mode === 'lacp-static') {
            ...
            if (timeout && timeout !== 'slow') {
                aggInterfaceCli += ` lacp timeout ${timeout}\n`;
            }
        }
        
        if (interfaceMode === 'l3') {
            aggInterfaceCli += ` undo portswitch\n`;
        } else if (interfaceMode === 'access' && accessVlan) {
            aggInterfaceCli += ` port link-type access\n port default vlan ${accessVlan}\n`;
        } else if (interfaceMode === 'trunk') {
        ...
        members.forEach(member => {
            if (member.name) {
                memberInterfacesCli += `interface ${member.name}\n`;
                if (mode === 'lacp-static' && member.portPriority && member.portPriority !== '32768') {
                    memberInterfacesCli += ` lacp priority ${member.portPriority}\n`;
                }
    ...
    // --- H3C ---
        ...
        if (mode === 'dynamic' && systemPriority && systemPriority !== '32768') {
            globalCli += `lacp system-priority ${systemPriority}\n`;
        }
        if (loadBalanceAlgorithm) {
            globalCli += `link-aggregation global load-sharing mode ${loadBalanceAlgorithm}\n`;
        }
        ...
        if (!isL3Interface) {
            if (interfaceMode === 'access' && accessVlan) {
                aggInterfaceCli += ` port access vlan ${accessVlan}\n`;
            } else if (interfaceMode === 'trunk') {
                aggInterfaceCli += ` port link-type trunk\n`;
                if (trunkNativeVlan) aggInterfaceCli += ` port trunk pvid vlan ${trunkNativeVlan}\n`;
        ...
        if (mode === 'dynamic') {
             aggInterfaceCli += ` link-aggregation mode dynamic\n`;
        } else if (mode === 'static') {
             aggInterfaceCli += ` link-aggregation mode static\n`;
        }
        aggInterfaceCli += `quit\n`;
        ...
        members.forEach(member => {
            if (member.name) {
                ...
                if (mode === 'dynamic') {
                    if (member.portPriority && member.portPriority !== '32768') {
                        memberInterfacesCli += ` link-aggregation port-priority ${member.portPriority}\n`;
                    }
                    if (member.lacpMode === 'passive') {
                        memberInterfacesCli += ` lacp mode passive\n`;
                    } else { // active is default, this command reverts to it
                        memberInterfacesCli += ` undo lacp mode\n`;
                    }
                    if (member.lacpPeriod === 'short') {
                        memberInterfacesCli += ` lacp period short\n`;
                    }
                }
                memberInterfacesCli += `quit\n`;
            }
    ...
    const finalCli = [
        globalCli.trim(),
        aggInterfaceCli.trim(),
        memberInterfacesCli.trim()
    ].filter(Boolean).join('\n\n');

    return { cli: finalCli, explanation: "Link Aggregation configuration generated locally." };
};
