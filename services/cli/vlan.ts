import { VLANConfig, VLANInterface, ACL, Vendor } from '../../types';

export const generateVlanCli = (vendor: string, deviceType: string, config: VLANConfig, acls: ACL[] = []): { cli: string; explanation: string } => {
            ...
            if (vlan.enableDHCP) {
                if (vendorLower === 'cisco') {
                    if (vlan.dhcpMode === 'relay' && vlan.dhcpServerIP) {
                        interfaceBlock += ` ip helper-address ${vlan.dhcpServerIP}\n`;
                    }
                } else if (vendorLower === 'huawei') {
                    if (vlan.dhcpMode === 'relay' && vlan.dhcpServerIP) {
                        interfaceBlock += ` dhcp select relay\n`;
                        interfaceBlock += ` dhcp relay server-ip ${vlan.dhcpServerIP}\n`;
                    ...
                    if (vlan.dhcpMode === 'relay' && vlan.dhcpServerIP) {
                        interfaceBlock += ` dhcp select relay\n`;
                        interfaceBlock += ` dhcp relay server-select ${vlan.dhcpServerIP}\n`;
                    } else if (vlan.dhcpMode === 'global' && vlan.selectedPool) {
